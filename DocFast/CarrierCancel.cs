...
namespace DocFast.WebObjects
{
    public partial class actions
    {
        public void DoCarrierCancel()
        {
            ...
            ActionClick.Click();
            WebElement Cancel = new WebElement().ByText("Cancel Delivery");
            Cancel.Click();

            Thread.Sleep(4000);

            WebElement CancelReason = new WebElement().ById("RequestSelected0");

            if (CancelReason.Displayed)
            {
                CancelReason.Select("23");

                //foreach (IWebElement option in CancelReason.ByTagName(SeleNUnit.WebObjects.Tags.TagNames.Option))
                //{
            ...
            DeclinedOfferReason0.SendKeys("Cancel test");
