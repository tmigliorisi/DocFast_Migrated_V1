    ...
    public partial class RT
    {
        private void CreateCDAC()
        {
            ...
            if (!newPol.CCQ) // PolicyEX ID Check
            {
                ...
                FirstName.SendKeys(DocFast.Resource.CDAC_PI_FN);

                IWebElement LastName = driver.FindElement(By.Id("LastName"));
                LastName.SendKeys(DocFast.Resource.CDAC_PI_LN);
            }
            ...
            if (!newPol.CCQ) // PolicyEX ID Check
            {
                if (newPol.Supplier != "Standard" && newPol.Supplier != "Standard NY")
                {
                    ...
                    if (DateTime.Now.Minute % 2 == 0)
                    {
                        ...
                        txtConsumerQuizAnswerChoiceText_3.SendKeys(DocFast.Resource.CDAC_PI_LN);
                    }
            ...
            if (newPol.Additional)
            {
                ...
                FirstNameAdd.SendKeys(DocFast.Resource.CDAC_PI_FN);

                IWebElement LastNameAdd = driver.FindElement(By.Id("NewConsumerLastName"));
                LastNameAdd.SendKeys(DocFast.Resource.CDAC_Add_LN);
            ...
            if (newPol.PrivateAttachment)
            {
                if (newPol.Server == "QA")
                {
                    ...
                    if (!newPol.CCQ)
                    {
                        new SelectElement(driver.FindElement(By.Id("PrivateConfidentialDocument_pdf_RecipientListCombo"))).SelectByText(DocFast.Resource.CDAC_PI_FN + " " + DocFast.Resource.CDAC_PI_LN);
                    }
                    else
                    {
            ...
            // Distributor
            driver.FindElement(By.Id("DistributorCheckBox")).Click();
            new SelectElement(driver.FindElement(By.Id("DistributorDropDown"))).SelectByValue(newPol.DistributorID);
            
            Thread.Sleep(2000);

            // Distributor Case Manager
            IWebElement CaseManagerID = driver.FindElement(By.XPath(".//*[@id='CaseManagerID']"));
            foreach (IWebElement option in CaseManagerID.FindElements(By.TagName("option")))
            {
                if (option.GetAttribute("value") == newPol.DCMID) // QA = 60 DCM_4, 56 DCMA_4, 333 Maggie's; UAT = 627 DCM_2, 624 DCMA_2
                {
                    option.Click();
                    break;
                }
