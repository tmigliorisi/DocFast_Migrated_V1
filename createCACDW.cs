    ...
    public partial class RT
    {
        private void CreateCACDW()
        {
            ...
            if (!newPol.CCQ) // PolicyEX ID Check
            {
                ...
                FirstName.SendKeys(DocFast.Resource.CACDW_PI_FN);

                IWebElement LastName = driver.FindElement(By.Id("LastName"));
                LastName.SendKeys(DocFast.Resource.CACDW_PI_LN);
            }
            ...
            if (!newPol.CCQ) // PolicyEX ID Check
            {
                if (newPol.Supplier != "Standard" && newPol.Supplier != "Standard NY")
                {
                    ...
                    if (DateTime.Now.Minute % 2 == 0)
                    {
                        ...
                        txtConsumerQuizAnswerChoiceText_3.SendKeys(DocFast.Resource.CACDW_PI_LN);
                    }
            ...
            if (newPol.Additional)
            {
                ...
                FirstNameAdd.SendKeys(DocFast.Resource.CACDW_PI_FN);

                IWebElement LastNameAdd = driver.FindElement(By.Id("NewConsumerLastName"));
                LastNameAdd.SendKeys(DocFast.Resource.CACDW_Add_LN);
            ...
            if (newPol.PrivateAttachment)
            {
                if (newPol.Server == "QA")
                {
                    ...
                    if (!newPol.CCQ)
                    {
                        new SelectElement(driver.FindElement(By.Id("PrivateConfidentialDocument_pdf_RecipientListCombo"))).SelectByText(DocFast.Resource.CACDW_PI_FN + " " + DocFast.Resource.CACDW_PI_LN);
                    }
                    else
                    {
